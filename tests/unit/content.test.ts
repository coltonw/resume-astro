...
describe("sections", () => {
  it("every section has a non-empty id and at least one paragraph", () => {
    for (const s of SECTIONS) {
      ...
      const total =
        (s.preIcon?.length ?? 0) + s.intro.length + (s.outro?.length ?? 0);
    ...
    for (const s of SECTIONS) {
      for (const ref of s.iconLine.icons) {
        if (isText(ref)) {
          ...
          expect(
            ICON_META[ref],
            `icon "${ref}" used in section "${s.id}" exists`,
          ).toBeDefined();
        }
      }
    }
...
describe("icons registry", () => {
  ...
  it("every referenced SVG file exists in components/svgs", async () => {
    ...
    const files = new Set(
      (
        await readdir(new URL("../../src/components/svgs", import.meta.url))
      ).map((f) => f.replace(/\.astro$/, "")),
    );
    for (const [key, icon] of Object.entries(ICON_META)) {
      expect(files.has(icon.svg), `${key} -> ${icon.svg}.astro exists`).toBe(
        true,
      );
    }
  });
