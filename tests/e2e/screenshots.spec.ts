...
test.describe('home page', () => {
  test('renders core museum content', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Voting', exact: true })).toBeVisible();
    await expect(page.getByRole('heading', { name: 'Making a Real Game' })).toBeVisible();
  ...
  test('full-page screenshot @ desktop and mobile', async ({ page }, testInfo) => {
    await page.goto('/');
...
test.describe('tldr page', () => {
  ...
  test('ships no hydration JS (only the prefetch runtime)', async ({ page }) => {
    ...
    const hydrationOrFramework = scriptSrcs.filter(
      (u) => /client\.svelte|EmbeddedVideo|template/.test(u),
    );
